...
impl ProgressDownloadModel {
    ...
    fn final_pause() -> Cmd {
        tick(Duration::from_millis(750), |_| Box::new(FinalPauseMsg) as Msg)
    }
}
...
impl Model for ProgressDownloadModel {
    fn init() -> (Self, Option<Cmd>) {
        ...
        let tick_cmd = tick(Duration::from_millis(100), |_| Box::new(DownloadTickMsg) as Msg);
        (model, Some(tick_cmd))
    }
            ...
            match self.downloader.tick() {
                Ok(is_complete) => {
                    ...
                    if is_complete {
                        ...
                        cmds.push(tick(Duration::from_millis(100), |_| Box::new(DownloadTickMsg) as Msg));
                    }
