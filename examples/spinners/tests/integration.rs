...
#[path = "../main.rs"]
mod spinners_main;
use spinners_main::{SpinnerStyle, SpinnerTickMsg, SpinnersModel};
...
fn test_all_spinner_styles_have_properties() {
    ...
    for style in styles {
        ...
        assert!(
            ...
            "Style {:?} should have a description",
            style
        );
    }
}
...
fn test_specific_spinner_characteristics() {
    ...
    assert_ne!(
        SpinnerStyle::Pulse.interval(),
        SpinnerStyle::Globe.interval()
    );
    assert_ne!(
        SpinnerStyle::Dots.interval(),
        SpinnerStyle::Points.interval()
    );
}
