...
mod tests {
    ...
    fn key_msg(key: KeyCode) -> Msg {
        Box::new(KeyMsg { key, modifiers: KeyModifiers::empty() }) as Msg
    }
    ...
    fn test_model_initialization() {
        ...
        assert_eq!(model.terminal_width, 80, "Default terminal width should be 80");
    ...
    fn test_key_bindings_creation() {
        ...
        assert!(binding_keys.contains(&"↓/j"), "Should have down key binding");
        assert!(binding_keys.contains(&"←/h"), "Should have left key binding");
        assert!(binding_keys.contains(&"→/l"), "Should have right key binding");
        assert!(binding_keys.contains(&"?"), "Should have help toggle binding");
    ...
    fn test_key_binding_matching() {
        ...
        assert!(model.find_key_binding(KeyCode::Up).is_some(), "Up arrow should match");
        assert!(model.find_key_binding(KeyCode::Char('k')).is_some(), "k should match up");
        assert!(model.find_key_binding(KeyCode::Down).is_some(), "Down arrow should match");
        assert!(model.find_key_binding(KeyCode::Char('j')).is_some(), "j should match down");
        assert!(model.find_key_binding(KeyCode::Left).is_some(), "Left arrow should match");
        assert!(model.find_key_binding(KeyCode::Char('h')).is_some(), "h should match left");
        assert!(model.find_key_binding(KeyCode::Right).is_some(), "Right arrow should match");
        assert!(model.find_key_binding(KeyCode::Char('l')).is_some(), "l should match right");
        
        // Test action key bindings
        assert!(model.find_key_binding(KeyCode::Char('?')).is_some(), "? should match help");
        assert!(model.find_key_binding(KeyCode::Char('q')).is_some(), "q should match quit");
        assert!(model.find_key_binding(KeyCode::Esc).is_some(), "Esc should match quit");
        assert!(model.find_key_binding(KeyCode::Char('c')).is_some(), "c should match quit");
        
        // Test non-matching keys
        assert!(model.find_key_binding(KeyCode::Char('x')).is_none(), "x should not match");
        assert!(model.find_key_binding(KeyCode::Enter).is_none(), "Enter should not match");
    }
    ...
    fn test_help_toggle() {
        ...
        assert!(model.help_expanded, "Help should be expanded after pressing ?");
        assert!(cmd.is_none());
        
        // Press ? again to collapse help
        let cmd = model.update(key_msg(KeyCode::Char('?')));
        assert!(!model.help_expanded, "Help should be collapsed after pressing ? again");
        assert!(cmd.is_none());
    }
    ...
    fn test_window_resize() {
        ...
        assert_eq!(model.terminal_width, 120, "Terminal width should be updated");
        assert!(cmd.is_none(), "Window resize should not return command");
        
        // Test another resize
        let cmd = model.update(window_size_msg(60, 20));
        assert_eq!(model.terminal_width, 60, "Terminal width should be updated again");
        assert!(cmd.is_none(), "Window resize should not return command");
    }
    ...
    fn test_view_initial_state() {
        ...
        assert!(view.contains("Waiting for input..."), "Should show waiting message initially");
        assert!(view.contains("? toggle help"), "Should show help toggle in collapsed view");
        assert!(view.contains("q quit"), "Should show quit option in collapsed view");
        assert!(!view.contains("move up"), "Should not show navigation help in collapsed view");
    }
    ...
    fn test_view_after_key_press() {
        ...
        let view = model.view();
        assert!(view.contains("You chose:"), "Should show chosen key message");
        assert!(!view.contains("Waiting for input..."), "Should not show waiting message");
    }
    ...
    fn test_view_expanded_help() {
        ...
        let view = model.view();
        assert!(view.contains("move up"), "Should show navigation help in expanded view");
        assert!(view.contains("move down"), "Should show navigation help in expanded view");
        assert!(view.contains("move left"), "Should show navigation help in expanded view");
        assert!(view.contains("move right"), "Should show navigation help in expanded view");
        assert!(view.contains("toggle help"), "Should show help toggle in expanded view");
        assert!(view.contains("quit"), "Should show quit option in expanded view");
    }
    ...
    fn test_short_help_bindings() {
        ...
        assert!(help_keys.contains(&"?"), "Short help should include help toggle");
    ...
    fn test_full_help_bindings() {
        ...
        assert!(nav_help_keys.contains(&"↑/k"), "Navigation should include up key");
        assert!(nav_help_keys.contains(&"↓/j"), "Navigation should include down key");
        assert!(nav_help_keys.contains(&"←/h"), "Navigation should include left key");
        assert!(nav_help_keys.contains(&"→/l"), "Navigation should include right key");
        
        let action_help_keys: Vec<&str> = action_keys.iter().map(|b| b.help_key).collect();
        assert!(action_help_keys.contains(&"?"), "Actions should include help toggle");
        assert!(action_help_keys.contains(&"q"), "Actions should include quit");
    }
    ...
    fn test_format_help_line() {
        ...
        assert!(formatted.contains("? toggle help"), "Should contain help binding");
    ...
    fn test_format_help_columns() {
        ...
        assert!(formatted.contains("move up"), "Should contain navigation descriptions");
        assert!(formatted.contains("toggle help"), "Should contain action descriptions");
        assert!(formatted.contains("  "), "Should have proper column spacing");
    }
    ...
    fn test_format_help_columns_narrow_width() {
        ...
        assert!(!formatted.is_empty(), "Should still produce output for narrow width");
        assert!(formatted.contains("...") || formatted.len() < 100, "Should truncate or be short for narrow width");
    }
    ...
    fn test_unknown_key_no_effect() {
        ...
        assert_eq!(initial_state, final_state, "Unknown key should not change state");
    ...
    fn test_key_binding_symbols() {
        ...
        assert_eq!(down_binding.symbol, "↓", "Down key should have down arrow symbol");
        
        let left_binding = model.find_key_binding(KeyCode::Left).unwrap();
        assert_eq!(left_binding.symbol, "←", "Left key should have left arrow symbol");
        
        let right_binding = model.find_key_binding(KeyCode::Right).unwrap();
        assert_eq!(right_binding.symbol, "→", "Right key should have right arrow symbol");
    }
}